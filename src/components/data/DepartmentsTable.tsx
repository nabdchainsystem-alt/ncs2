...
export default function DepartmentsTable() {
  ...
  const { rows, isLoading, isError, error, mutate } = useDepartments({ page: 1, pageSize: 10 });
  ...
  const resetForm = () => {
    setFormState({ name: "", code: "", location: "" });
    setFormError(null);
    setSelectedId(null);
    setMode("create");
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setFormError(payload.message ?? "Failed to save department");
        return;
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      setDeleteError(null);
      const response = await fetch(`/api/departments/${deleteTarget.id}`, {
        method: "DELETE",
      });

      if (!response.ok) {
        const payload = await response.json().catch(() => ({}));
        setDeleteError(payload.message ?? "Failed to delete department");
        return;
      }
  ...
  return (
    <>
      <TableCard
        title="Departments"
        subtitle="Manage and add new departments"
      ...
      <Dialog open={open} handler={handleClose} size="md">
        <DialogHeader className="tw-flex tw-flex-col tw-items-start tw-gap-1 tw-rounded-t-xl tw-border-b tw-border-blue-gray-50">
          ...
          <Typography variant="small" className="!tw-font-normal !tw-text-blue-gray-500">
            Provide department details below.
          </Typography>
        </DialogHeader>
